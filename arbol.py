...
def obtener_arbol_izq_y_der_de_un_nodo_x(arbol, clave):
    ...
    if arbol is not None:
        ...
        elif arbol_izquierdo is None and arbol_izquierdo is None:   #para q va a seguir comparando si ya encontro el nodo (creo q esta parte no funciona ya que si lo encuentra ya ni entre al elif...quizas colocando el else es suficiente)
...
def eliminar_personaje(arbol, clave, previo=None, hijo=None):   #(RECORDAR: si se quiere modificar un nodo debemos eliminarlo y modificarlo y luego insertarlo de nuevo)
    x, datos = None, None
    if arbol['info'] is not None:
...
#################-----METODOS PARA EL PARCIAL 07/11/22  ----------##########################
def cargar_nuevo_personaje(arbol,nombre,altura,peso):
    insertar_nodo(arbol,nombre,{'altura':altura,'peso':peso})

def modificar_el_campo_nombre_de_x_jedi(arbol, nombre, nuevo_nombre): #como el arbol esta ordenado por el campo info tendremos q eliminar el nodo y cargarlo de nuevo con la nueva info
    info, datos =eliminar_nodo(arbol,nombre)
    if info is not None:
        insertar_nodo(arbol, nuevo_nombre, datos)

def modificar_el_campo_altura_de_x_jedi(arbol, nombre, nueva_altura): 
    info, datos =eliminar_nodo(arbol,nombre)
    if info is not None:
        datos['altura']= nueva_altura
        insertar_nodo(arbol, info, datos)

def modificar_el_campo_peso_de_x_jedi(arbol, nombre, nuevo_peso): 
    info, datos =eliminar_nodo(arbol,nombre)
    if info is not None:
        datos['peso']= nuevo_peso
        insertar_nodo(arbol, info, datos)

def mostrar_todas_las_info_de_jedi_x(arbol, jedi): 
    if arbol is not None:
        mostrar_todas_las_info_de_jedi_x(arbol['izq'], jedi)
        if arbol['info'].upper() == jedi.upper():
                print(arbol['info'],'datos: ',arbol['datos'])
        mostrar_todas_las_info_de_jedi_x(arbol['der'], jedi)

def mostrar_alfabeticamente_todos_los_personajes_jedis_q_midan_menos_q_altura_x(arbol,altura):    
    if arbol is not None:
        mostrar_alfabeticamente_todos_los_personajes_jedis_q_midan_menos_q_altura_x(arbol['izq'],altura)
        if arbol['datos']['altura']<altura:
            print(arbol['info']," datos: ",arbol['datos'])
        mostrar_alfabeticamente_todos_los_personajes_jedis_q_midan_menos_q_altura_x(arbol['der'],altura)

def mostrar_alfabeticamente_todos_los_personajes_jedis_q_pesan_mas_de_x_kilos(arbol,peso):    
    if arbol is not None:
        mostrar_alfabeticamente_todos_los_personajes_jedis_q_pesan_mas_de_x_kilos(arbol['izq'],peso)
        if arbol['datos']['peso']>peso:
            print(arbol['info']," datos: ",arbol['datos'])
        mostrar_alfabeticamente_todos_los_personajes_jedis_q_pesan_mas_de_x_kilos(arbol['der'],peso)

def jedi_x_esta_cargado(arbol, jedi):
    esta_cargado = False 
    if arbol is not None:
        esta_cargado = jedi_x_esta_cargado(arbol['izq'], jedi)
        if arbol['info'].upper() == jedi.upper():
            esta_cargado = True
        elif esta_cargado != True:
            esta_cargado = jedi_x_esta_cargado(arbol['der'], jedi)
    return esta_cargado

def nodo_de_jedi_x_(arbol, jedi):
    nodo = None
    if arbol is not None:
        nodo = nodo_de_jedi_x_(arbol['izq'], jedi)
        if arbol['info'].upper() == jedi.upper():
            nodo = arbol
        elif nodo == None:
            nodo = nodo_de_jedi_x_(arbol['der'], jedi)
    return nodo

def mostrar_altura_del_nodo_de_jedi_x(arbol,jedi):  
    nodo = nodo_de_jedi_x_(arbol,jedi) 
    if nodo is not None:
        print(nodo['altura'])

def nodo_de_jedi_x_es_hoja_intermedio_o_raiz(arbol,jedi):
    nodo = nodo_de_jedi_x_(arbol, jedi)
    if nodo is not None:
        altura_de_arbol = arbol['altura']
        if nodo['altura'] == altura_de_arbol:
            print('es raiz')
        elif nodo['altura'] == 0:
            print('es hoja')
        else:
...
"""
print("")
print("INORDEN: (del más chico al más grande)")
inorden(arbol)
print("")
print("inorden_nrr: (creo q es con datos extras por ejemplo un vector)")
inorden_nrr(arbol)
print("")
print("postorden: (del más grande al más chico)")
postorden(arbol)
print("")
print("busqueda:")
print(busqueda(arbol,3))
print("")
print("eliminar_nodo: (elimina un nodo)")
print(eliminar_nodo(arbol,1))
print(arbol)
print("")
print("por_nivel: (muestra primero la raiz luego su hijo izquierdo y finalmente su hijo derecho...y asi sigue)")
por_nivel(arbol)
print("")
print("preorden: (muestra primero la raiz luego su altura despues su hijo izquierdo y finalmente su hijo derecho...y asi sigue)")
preorden(arbol)
"""
# insertar_nodo(arb, 6)
# insertar_nodo(arb, 7)
