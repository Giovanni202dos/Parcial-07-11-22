...
class Arista():
    ...
    def obetener_arista_con_peso_mayor_a_x_numero(self,numero):
        aux = self.__inicio
        vector = []
        while (aux is not None):
            #print(aux.info, aux.peso)
...
class Grafo():
    ...
    def obtener_vector_con_los_jedis_q_tienen_mas_a_x_episodios_con_otro(self,canti_epi):
        aux = self.__inicio
        lista = []
        while (aux is not None):
            #print(aux.info)
            vec=self.obtener_aristas_con_peso_mayor_a_x_canti_de_epi(aux.info, canti_epi)
            #print('  ',vec)
            if len(vec)>0:
                conexion ={aux.info:vec}
                lista.append(conexion)
            aux = aux.sig
        return lista

    # DEVUELVE UN VECTOR CON TODAS LAS ARISTAS CON SU PESO
    def obtener_aristas_con_peso_mayor_a_x_canti_de_epi(self, origen,numero): # FUNCIONA PARA GRAFOS NO DIRIGIDOS (aun no lo probe para dirigidos)
        vert_origen = self.busqueda_vertice(origen)
        if vert_origen:
            return vert_origen.adyacentes.obetener_arista_con_peso_mayor_a_x_numero(numero)
